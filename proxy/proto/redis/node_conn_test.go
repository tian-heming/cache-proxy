...
func TestNodeConnNewNodeConn(t *testing.T) {
	//为backend redis服务器建立和proxy服务器的socket端到端的连接通道
...
//把指定的cmd消息请求体发往(write conn)后端redis，并接收read后端的响应
func TestNodeConnWriteOk(t *testing.T) {
	conn := libnet.NewConn(mockconn.CreateMockConn([]byte("iam test bytes 24 length"), 1), time.Second, time.Second)
	nc := newNodeConn("baka", "127.0.0.1:12345", conn)
...
func TestNodeConnWriteHasErr(t *testing.T) {
	...
	nc := newNodeConn("baka", "127.0.0.1:12345", conn)
	ncc := nc.(*nodeConn)
	ec := libnet.NewConn(mockconn.CreateMockConn(nil, 1), time.Second, time.Second)
...
//读取后端发回复过来的数据，消息msg挈带请求newRequest来回传送
...
func TestReadWithBadAssert(t *testing.T) {
	nc := newNodeConn("baka", "127.0.0.1:12345", libnet.NewConn(mockconn.CreateMockConn([]byte(":123\r\n"), 1), time.Second, time.Second))
	msg := proto.NewMessage()
	msg.WithRequest(&mockCmd{})
	err := nc.Read(msg)
	assert.Error(t, err)
...
func TestReadHasErr(t *testing.T) {
	nc := newNodeConn("baka", "127.0.0.1:12345", libnet.NewConn(mockconn.CreateMockConn([]byte(":123\r\n"), 1), time.Second, time.Second))
	ncc := nc.(*nodeConn)
	ec := libnet.NewConn(mockconn.CreateMockConn([]byte(":123\r\n"), 1), time.Second, time.Second)
