...
// resp通信协议的请求和回复
//redis消息数据对应的`Request`接口实现，包含resp
// 各自redis 声明式key 指令的请求或回复 体
//https://github.com/bilibili/overlord/blob/master/doc/wiki-cn/proxy/design.md
import (
	"bytes"
	errs "errors"
	"sync"
...
// getReq get the request from reqPool
//从池子里获取一个对象，如果没有就执行池子里的New函数创建一个对象
func getReq() *Request {
