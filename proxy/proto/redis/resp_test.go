...
func TestRespDecode(t *testing.T) {
	...
	for _, tt := range ts {
		t.Run(tt.Name, func(t *testing.T) {
			//mockconn.CreateConn 模拟连接处理服务器的连接，默认模拟该连接包含数据tt.Bytes，通信一次
			conn := libnet.NewConn(mockconn.CreateMockConn(tt.Bytes, 1), time.Second, time.Second)
			r := &resp{} //单个resp协议项，包含内嵌数组协议项
			r.reset()
...
func TestRespEncode(t *testing.T) {
	...
	for _, tt := range ts {
		t.Run(tt.Name, func(t *testing.T) {
			//mockconn.CreateConn 这个连接是模拟 连接 处理服务器的连接，该连接初始时没有数据nil，通信一次，由客户端发数据给它
