...
func TestRequestNewRequest(t *testing.T) {
	...
	br := bufio.NewReader(conn, bufio.Get(1024)) //缓存连接对象 【4层tcp层 建立连接传输流数据】
	br.Read()                                    //读取io.Reader实例的数据到br缓存器区里，直到br写缓存区满(size:1024)
	req := getReq()                              //获取resp req请求对象，该对象封装了完整的resp请求和回复对象，类似gin的gin.context对象(该对象做了临时池复用优化)
	err := req.resp.decode(br)                   //将br返回的缓存区数据解码到req.resp对象里 【5+层app层 封装连接到缓存区，去编解码数据到指定序列化协议对象，这个对象是频繁对象，需要适当优化】
	assert.Nil(t, err)
